// prueba_github.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, char *argv[]) {
    ...
    std::string wordfile = "input1.txt";
    trie_t theTrie;
    std::cout << "Building trie...\n";
    auto start = clock();
    buildDictionaryTrie(theTrie, wordfile);
		std::cout<<(clock()-start)/(float)CLOCKS_PER_SEC<<"s"<<std::endl;
